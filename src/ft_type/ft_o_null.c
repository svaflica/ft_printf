...
/*   Updated: 2019/05/14 14:42:22 by qclubfoo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			prec_pos_null(char *str, t_param list, t_buf *buf)
{
	if (list.minus == 0)
	{
		...
		if (list.width > list.precision/*  && list.precision > 0 */ && list.zero == 0)
			ft_buf_add_s(buf, ft_memnew(list.width - (list.hasht == 1 && list.precision < 1) - list.precision, ' '), 0);
		else if (list.width > list.precision/*  && list.precision > 0 */ && list.zero == 1)
			ft_buf_add_s(buf, ft_memnew(list.width - list.hasht - list.precision, '0'), 0);
