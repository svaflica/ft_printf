...
/*   Updated: 2019/05/13 16:46:34 by qclubfoo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			prec_neg(char *str, t_param list, t_buf *buf)
{
	...
	tmp = ft_strlen(str);
	// *str == '0' && tmp == 1 ? tmp = 0 : 0;
	if (list.minus == 0 && list.zero == 1 && list.width - list.hasht > tmp && list.precision <= -1)
	...
	list.hasht == 1 && *str != '0'/* tmp != 0 */ ? ft_buf_add_c(buf, '0') : NULL;
